...
def main():
    ...
    repo = os.environ.get("REPO", "faserf/ha-moneymanager").lower()
    ...
    for path in ["custom_components/moneymanager/manifest.json", "pyproject.toml"]:
        try:
    ...
    for f in changed_files:
        if f.startswith("custom_components/moneymanager/translations/"):
            translation_count += 1
    ...
    body_parts = [
        f"# MoneyManager {version}  {channel_badge}",
        "",
        prerelease_note,
        "## 📋 What's Changed",
        "",
